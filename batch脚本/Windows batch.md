...
其中Options是命令选项，须以/开头，后面跟选项（可以把多个选项连在一起或每个选项自己跟自己的参数）
如   del /f /q /s tmp
```
...
```powershell
batch也可以使用管道，用来把命令连在一起
```

```powershell
变量使用：
定义：set file = "D:\log.txt"
引用：用两个百分号%括起来  echo %file%
```
...
echo %p:~6,3%           #显s示第 6 个字符以后的 3 个字符，即 < span class="title">aa2
